...
<div class="mb-4 d-flex justify-content-between align-items-start">
    ...
    <div class="text-end">
        ...
        <form method="GET" class="d-flex justify-content-end">
            ...
            <select name="tahun" class="form-select form-select-sm" style="width: auto;" onchange="this.form.submit()" aria-label="Pilih Tahun">
                <?php for ($y = date('Y'); $y >= date('Y') - 5; $y--): ?>
                    <option value="<?php echo $y; ?>" <?php echo ($filter_tahun == $y) ? 'selected' : ''; ?>><?php echo $y; ?></option>
                <?php endfor; ?>
            </select>
            <noscript><button type="submit" class="btn btn-sm btn-secondary ms-2">Filter</button></noscript>
        </form>
    </div>
</div>
...
<!-- Panel Absensi untuk Karyawan -->
<?php if (!$is_admin): ?>
    <div class="card shadow-sm mb-4 border-start border-primary border-4">
        <div class="card-body">
            <div class="row align-items-center">
                <div class="col-md-7">
                    <h4 class="card-title mb-2">Absensi Hari Ini</h4>
                ...
                <div class="col-md-5 text-md-end mt-3 mt-md-0">
                    <?php if ($status_absensi_hari_ini_karyawan == "Belum Absen" || $status_absensi_hari_ini_karyawan == "Pengajuan Ditolak"): ?>
                        <div class="btn-group" role="group" aria-label="Aksi Absensi">
                            <a href="../fungsi/proses_pengajuan_hadir.php" class="btn btn-primary btn-lg">
            ...
            <table class="table table-hover table-striped align-middle">
                <thead class="table-light">
                    <tr>
                        ...
                        <th class="text-center">Status Diajukan</th>
                ...
                <tbody>a
                    ...
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($pengajuan['username']); ?></td>
                        <td><?php echo date('d M Y', strtotime($pengajuan['tanggal'])); ?></td>
                        <td class="text-center"><?php echo $pengajuan['jam_masuk'] ? date('H:i', strtotime($pengajuan['jam_masuk'])) : '-'; ?></td>
                        <td class="text-center">
                            <?php 
                            $status_badge_class = 'bg-secondary';
                            if ($pengajuan['status_diajukan'] == 'Hadir') $status_badge_class = 'bg-success';
                            if ($pengajuan['status_diajukan'] == 'Izin') $status_badge_class = 'bg-info text-dark';
                            if ($pengajuan['status_diajukan'] == 'Sakit') $status_badge_class = 'bg-warning text-dark';
                            ?>
                        ...
                        <td class="text-center">
                            <?php 
                            $kondisi_badge = '-';
                            if ($pengajuan['kondisi_masuk'] == 'Tepat Waktu') {
                                $kondisi_badge = "<span class='badge bg-success'>Tepat Waktu</span>";
                            } elseif ($pengajuan['kondisi_masuk'] == 'Terlambat') {
                        ...
                        <td class="text-center">
                            <a href="../fungsi/proses_persetujuan_absensi.php?id=<?php echo $pengajuan['id']; ?>&aksi=setujui" class="btn btn-sm btn-success me-1" onclick="return confirm('Anda yakin ingin MENYETUJUI pengajuan ini?')">
                                <i class="bi bi-check-lg"></i> Setujui
                            </a>
                            <a href="../fungsi/proses_persetujuan_absensi.php?id=<?php echo $pengajuan['id']; ?>&aksi=tolak" class="btn btn-sm btn-danger" onclick="return confirm('Anda yakin ingin MENOLAK pengajuan ini?')">
            ...
            <div class="col-md-6 col-lg-6">
                <div class="card text-white bg-primary shadow-sm h-100"> 
                    <div class="card-body d-flex justify-content-between align-items-center">
                        <div><h3 class="card-title mb-0"><?php echo $total_karyawan; ?></h3><p class="card-text">Total Karyawan Aktif</p></div>
    ...
    if (ctxHadirPerHari) {
        new Chart(ctxHadirPerHari, {
            ...
            options: {
                ...
                scales: { 
                    y: { beginAtZero: true, ticks: { precision: 0, stepSize: 1 }, title: { display: true, text: 'Jumlah Kehadiran'}},
                    x: { title: { display: true, text: 'Tanggal (Hari ke-)'}}
                },
